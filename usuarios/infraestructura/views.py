...
class BuscarUsuarioPorTelefonoView(APIView):
    ...
    def get(self, request):
        ...
        if not telefono:
            return Response({"error": "Teléfono no proporcionado"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if usuario:
                ...
                direcciones = list(usuario.direcciones.order_by('-es_predeterminada', '-fecha_creacion').values_list('direccion', flat=True))
